...
const IMDBTitleDetailsResolver_1 = require("./IMDBTitleDetailsResolver");
const titlesToTest = [
    {
        ...
        genres: [enums_1.Genre.Action, enums_1.Genre.Adventure, enums_1.Genre.Fantasy, enums_1.Genre.SciFi],
        directors: {
            length: 1,
        ...
        mainType: enums_1.TitleMainType.Movie,
        ...
        casts: {
            ...
            tests: [
                ...
                {
                    index: null,
                    finderF: (item) => item.name.toLowerCase() === "kevin dorman",
                    name: "kevin dorman",
                    sourceId: null,
                    rolesLength: 2,
        ...
        posterImageUrl: "https://m.media-amazon.com/images/M/MV5BNjA3NGExZDktNDlhZC00NjYyLTgwNmUtZWUzMDYwMTZjZWUyXkEyXkFqcGdeQXVyMTU1MDM3NDk0.jpg",
    ...
    },
    {
        ...
        genres: [enums_1.Genre.Action, enums_1.Genre.Adventure, enums_1.Genre.Fantasy, enums_1.Genre.Drama],
        directors: {
            length: 19,
        ...
        mainType: enums_1.TitleMainType.Series,
        ...
        firstCountriesOfOrigin: "united states",
        // till here
        posterImageUrl: "https://m.media-amazon.com/images/M/MV5BYTRiNDQwYzAtMzVlZS00NTI5LWJjYjUtMzkwNTUzMWMxZTllXkEyXkFqcGdeQXVyNDIzMzcwNjc@.jpg",
        allImagesMinLength: 48,
        boxofficeBudget: 0,
        openingAmount: 0,
        openingDateYear: 0,
    ...
    },
    {
        ...
        name: "inside",
        mainType: enums_1.TitleMainType.TVSpecial,
        sourceId: "tt14544192",
    },
    {
        ...
        genres: [enums_1.Genre.Documentary, enums_1.Genre.Comedy, enums_1.Genre.Short],
    },
];
describe("imdb title details resolver", () => {
    titlesToTest.forEach((testData) => {
        test(`check ${testData.name} ${testData.titleYear} result`, async () => {
            const resolver = new IMDBTitleDetailsResolver_1.IMDBTitleDetailsResolver(testData.url);
            const result = await resolver.getDetails();
            expect(result).not.toBe(undefined);
            if (!result) {
                return;
            }
            expect(result.detailsLang).toBe(enums_1.Language.English);
            expect(result.mainSource.sourceId).toBe(testData.sourceId);
            expect(result.mainSource.sourceType).toBe(enums_1.Source.IMDB);
            expect(result.mainSource.sourceUrl).toBe(testData.url);
            if (testData.sourcesMinLength !== undefined) {
                expect(result.allSources.length).toBeGreaterThanOrEqual(testData.sourcesMinLength);
            }
            ...
            if (testData.mainRate !== undefined) {
                ...
                expect(mainRate.rateSource).toBe(enums_1.Source.IMDB);
            }
            if (testData.metaScore !== undefined) {
                expect(result.allRates.find((i) => i.rateSource === enums_1.Source.MetaCritics)
                    ?.rate).toBe(testData.metaScore);
            }
            if (testData.producersMinLength !== undefined) {
                expect(result.producers.length).toBeGreaterThanOrEqual(testData.producersMinLength);
            }
            if (testData.allRatesMinLength !== undefined) {
                expect(result.allRates.length).toBeGreaterThanOrEqual(testData.allRatesMinLength);
            }
            if (testData.dates !== undefined) {
                ...
                expect(result.dates.startCountry.toLocaleLowerCase()).toBe(testData.dates.startCountry);
            }
            if (testData.allReleaseDatesMinLength !== undefined) {
                expect(result.allReleaseDates.length).toBeGreaterThanOrEqual(testData.allReleaseDatesMinLength);
            }
            ...
            if (testData.languages !== undefined) {
                Array.from(Array(testData.languages?.length)).forEach((v, i) => {
                    expect(result.languages[i].toLocaleLowerCase()).toBe(testData.languages[i]);
                });
            }
            if (testData.firstCountriesOfOrigin !== undefined) {
                expect(result.countriesOfOrigin[0].toLocaleLowerCase()).toBe(testData.firstCountriesOfOrigin);
            }
            if (testData.posterImageUrl !== undefined) {
                expect(result.posterImage.url).toContain(".jpg");
            }
            if (testData.allImagesMinLength !== undefined) {
                expect(result.allImages.length).toBeGreaterThanOrEqual(testData.allImagesMinLength);
            }
            if (testData.boxofficeBudget) {
                ...
                expect(result.boxOffice?.opening.date.getFullYear()).toBe(testData.openingDateYear);
                if (testData.worldWideSellMin !== undefined) {
                    expect(result.boxOffice?.worldwide).toBeGreaterThanOrEqual(testData.worldWideSellMin);
                }
                if (testData.mainCountriesSellMin !== undefined) {
                    expect(result.boxOffice?.mainCountries.amount).toBeGreaterThanOrEqual(testData.mainCountriesSellMin);
                }
            }
            if (testData.firstProductionCompanyName !== undefined) {
                expect(result.productionCompanies[0].name.toLowerCase()).toBe(testData.firstProductionCompanyName);
            }
            if (testData.productionCompaniesLength !== undefined) {
                expect(result.productionCompanies.length).toBeGreaterThanOrEqual(testData.productionCompaniesLength);
            }
            if (testData.taglinesMinLength !== undefined) {
                expect(result.taglines.length).toBeGreaterThanOrEqual(testData.taglinesMinLength);
            }
            ...
            if (testData.keywordsMinLength !== undefined) {
                expect(result.keywords.length).toBeGreaterThanOrEqual(testData.keywordsMinLength);
            }
            if (testData.onOfKeywords !== undefined) {
                expect(result.keywords
                    .map((i) => i.toLowerCase())
                    .includes(testData.onOfKeywords)).toBe(true);
            }
            // posters & still frame images length
            if (testData.postersMinLength !== undefined) {
                expect(result.allImages.filter((i) => i.type === "poster" && !!i.url)
                    .length).toBeGreaterThanOrEqual(testData.postersMinLength);
            }
            if (testData.stillFrameMinLength !== undefined) {
                expect(result.allImages.filter((i) => i.type === "still_frame" && !!i.url)
                    .length).toBeGreaterThanOrEqual(testData.stillFrameMinLength);
            }
            ...
            if (testData.minNominations !== undefined) {
                expect(result.awardsSummary.totalNominations).toBeGreaterThan(testData.minNominations);
            }
            ...
            if (testData.spoilerQuotes !== undefined) {
                expect(result.quotes.filter((i) => i.isSpoiler).length).toBeGreaterThanOrEqual(testData.spoilerQuotes);
